(function() {
    ...
    console.log("🚀 Injected script loaded in main world");
            ...
            if (userInput) {
                try {
                    ...
                    if (!isValidReplacementData(replacementData)) {
                        ...
                        alert("Invalid data structure. coords and colors must be non-empty arrays and coords count must be exactly double the colors count.");
                    ...
                    const requests = tileGroups.map(group => {
                        const newUrl = url.replace(/\/s0\/pixel\/\d+\/\d+/, `/s0/pixel/${group.tileX}/${group.tileY}`);
                ...
                if (data && typeof data === 'string') {
                    ...
                    if (result.success && result.requests.length > 1) {
                        ...
                        if (req.url !== this._url) {
                            ...
                            console.warn("⚠️ XHR URL redirect not fully supported - may need page reload");
                        }
                        data = req.body;
                    }
                }
    ...
    console.log("✅ Request interceptor ready in main world");
})();